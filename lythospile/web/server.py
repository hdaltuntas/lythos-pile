"""
lythospile.web.server — the local HTTP server that serves the interface.
...
#: A mark for the browser tab: a cap on two piles through layered ground,
#: the load coming down on it
_FAVICON = (
    b'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">'
    b'<rect width="32" height="32" rx="7" fill="#F5F4ED"/>'
    b'<path d="M3 12h26v8H3z" fill="#E3C98F"/>'
    b'<path d="M3 20h26v9H3z" fill="#A7B8A0"/>'
    b'<path d="M7 9h18v4H7z" fill="#6B6A64"/>'
    b'<path d="M9 13h4v14H9zM19 13h4v14h-4z" fill="#8A8680"/>'
    b'<path d="M16 1v6" stroke="#C6613F" stroke-width="2.4" stroke-linecap="round"/>'
    b'<path d="M12.5 4.5 16 8l3.5-3.5" fill="none" stroke="#C6613F" stroke-width="2.2" '
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if route == "/api/socket":
                return self._json(SESSION.analyse_socket(data.get("values", {})))
    ...
    def _project(self, data: dict) -> None:
        ...
        self._download("project.pile", payload, "application/json; charset=utf-8")
...
def serve(host: str = "127.0.0.1", port: int = 8783, open_browser: bool = True,
          lang: str = "en") -> None:
