#!/usr/bin/env python3
"""Run Lythos Pile without installing it.

    python main.py                         opens the interface in a browser
    python main.py run project.pile     analyses a project file
    python main.py study project.pile   runs the study a project file defines
...
def explain(missing: list) -> None:
    print("Lythos Pile needs these Python packages, which are not installed:\n",
          file=sys.stderr)
    for name in missing:
...
def main() -> int:
    if sys.version_info < (3, 10):
        print(f"Lythos Pile needs Python 3.10 or later; this is "
    ...
    if gaps:
        explain(gaps)
        return 1
    from lythospile.cli import main as run_cli
