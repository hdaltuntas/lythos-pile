...
import lythospile
from lythospile import cli, forms
from lythospile.web import server
...
def test_the_version_is_the_same_in_the_package_and_the_distribution():
    ...
    assert declared.group(1) == lythospile.__version__


def test_the_static_files_are_declared_as_package_data():
    assert '"lythospile.web" = ["static/*"]' in pyproject()
...
def test_the_console_script_points_at_something_that_exists():
    assert 'lythos-pile = "lythospile.cli:main"' in pyproject()
...
def test_the_dependencies_are_the_ones_the_program_imports():
    ...
    package = os.path.join(ROOT, "lythospile")
...
def test_the_command_line_writes_runs_and_studies_a_project(tmp_path, capsys):
    ...
    assert "KAZIK TAŞIMA GÜCÜ SONUÇLARI" in capsys.readouterr().out
    assert cli.main(["socket", str(path), "-o", str(tmp_path / "s.html")]) == 0
    assert "ROCK-SOCKETED PILE" in capsys.readouterr().out
    assert (tmp_path / "s.html").stat().st_size > 10_000
